#!/usr/bin/env bash
# =============================================================================
# scripts/verify.sh — verify a downloaded Molao release artifact, fail-closed.
#
...
# WHAT IT IS FOR
# ──────────────
# A release publishes assets plus a `SHA256SUMS.txt` manifest. This script fetches
...
#     name as a regex — every "." in "molao-0.1.0-linux-x86_64" is a wildcard
...
# USAGE
# ─────
#   scripts/verify.sh --tag v0.1.0 molao-0.1.0-linux-x86_64
#   scripts/verify.sh --tag v0.1.0 --attest molao-0.1.0-linux-x86_64
#   scripts/verify.sh --repo vul-os/molao --tag v0.1.0 --out ~/Downloads ASSET
...
SELFTEST_ARTIFACT="molao-9.9.9-linux-x86_64"
...
trap cleanup EXIT
TMPDIR_V="$(mktemp -d "${TMPDIR:-/tmp}/molao-verify.XXXXXX")"
...
run_selftest() {
  ...
  expect_exit "SHA256SUMS.txt 404"                "$E_SUMS_FETCH"     yes -- "${V[@]}" --base-url "$base/nosums"     "$art"
  expect_exit "SHA256SUMS.txt is HTML (ctype)"    "$E_SUMS_HTML"      yes -- "${V[@]}" --base-url "$base/htmlsums"   "$art"
  expect_exit "SHA256SUMS.txt is HTML (sniffed)"  "$E_SUMS_HTML"      yes -- "${V[@]}" --base-url "$base/htmlsums2"  "$art"
  expect_exit "SHA256SUMS.txt empty"              "$E_SUMS_MALFORMED" yes -- "${V[@]}" --base-url "$base/emptysums"  "$art"
  expect_exit "SHA256SUMS.txt has no digests"     "$E_SUMS_MALFORMED" yes -- "${V[@]}" --base-url "$base/junksums"   "$art"
  expect_exit "SHA256SUMS.txt truncated"          "$E_SUMS_MALFORMED" yes -- "${V[@]}" --base-url "$base/truncsums"  "$art"
  ...
  mkdir -p "$ld"
  printf 'x' > "${ld}/${art}"
  expect_exit "local dir, no SHA256SUMS.txt"      "$E_SUMS_FETCH"     yes -- bash "$SELF_PATH" --dir "$ld" "$art"
  : > "${ld}/${MANIFEST}"
  expect_exit "local dir, empty SHA256SUMS.txt"   "$E_SUMS_MALFORMED" yes -- bash "$SELF_PATH" --dir "$ld" "$art"
