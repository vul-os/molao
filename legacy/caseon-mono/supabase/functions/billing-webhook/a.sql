...
BEGIN
    ...
    -- Look up or create subscription
...
BEGIN
    ...
    SELECT * INTO plan_record
    FROM plans
    WHERE id = subscription_record.plan_id AND is_active = true;
    ...
    IF plan_record.billing_cycle = 'monthly' THEN
        next_billing_date := NOW() + INTERVAL '1 month';
    ELSIF plan_record.billing_cycle = 'quarterly' THEN
        next_billing_date := NOW() + INTERVAL '3 months';
    ELSIF plan_record.billing_cycle = 'yearly' THEN
...
BEGIN
    ...
    -- Then reschedule active subscriptions
    FOR subscription_record IN 
        SELECT s.*, pm.id as payment_method_id
        FROM subscriptions s
        JOIN plans p ON p.id = s.plan_id
        JOIN payment_methods pm ON pm.firm_id = s.firm_id AND pm.is_default = true
        WHERE 
            s.status = 'active' 
...
BEGIN
    ...
    IF random() > 0.5 THEN
        ...
        SET 
            status = 'active',
            updated_at = NOW()
        WHERE firm_id = auth_record.firm_id
        AND status = 'past_due';
...
BEGIN
    ...
    -- Update subscription status instead of firm record
    UPDATE subscriptions
    SET
        status = 'past_due',
        updated_at = NOW()
...
-- Function to cancel all subscriptions for a firm
CREATE OR REPLACE FUNCTION cancel_firm_subscriptions(
  firm_id_param uuid
)
RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM firms WHERE id = firm_id_param) THEN
    RETURN false;
  END IF;

  -- Cancel all active subscriptions for this firm
  UPDATE subscriptions
  SET 
    status = 'canceled',
