...
-- Add role column to firm_invites table
ALTER TABLE firm_invites
ADD COLUMN role text NOT NULL CHECK (role IN ('partner', 'associate')) DEFAULT 'associate';

-- Update the unique constraint to include role
ALTER TABLE firm_invites
DROP CONSTRAINT IF EXISTS firm_invites_firm_id_email_status_key;

ALTER TABLE firm_invites
ADD CONSTRAINT firm_invites_firm_id_email_role_status_key 
UNIQUE(firm_id, email, role, status);
